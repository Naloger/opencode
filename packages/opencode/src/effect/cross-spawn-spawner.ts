...
export const make = Effect.gen(function* () {
  ...
  const stdios = (
    ...
    const mapIo = (x: NodeChildProcess.IOType | undefined) =>
      process.platform === "win32" && x === "pipe" ? "overlapped" : x
    const arr: Array<NodeChildProcess.IOType | undefined> = [
      mapIo(input(sin.stream)),
      mapIo(output(sout.stream)),
      mapIo(output(serr.stream)),
    ]
    ...
    for (const x of extra) arr[x.fd] = mapIo("pipe")
